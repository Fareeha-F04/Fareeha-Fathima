      ...
      <div className="container mx-auto px-4">
        ...
        <div className="space-y-12 max-w-6xl mx-auto">
          {projects.map((project, index) => (
            <div 
              key={index} 
              className={`reveal flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-6 items-center`}
            >
                  ...
                  {project.images.map((image, imgIdx) => (
                    <img
                      ...
                      className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${
                        hoveredImage === `${index}-${imgIdx}` 
                          ? 'z-20 scale-105' 
                          : imgIdx === 0 
                            ? 'z-10' 
                            : 'z-0 translate-x-4 translate-y-4 opacity-70'
                      }`}
                      onMouseEnter={() => setHoveredImage(`${index}-${imgIdx}`)}
